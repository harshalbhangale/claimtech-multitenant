...
const SignatureStep: React.FC = () => {
  ...
  // Updated handleSubmit function in SignatureStep
  ...
  //     // 🔥 CORRECT FIX: Extract reference from the correct nested path
  //     const reference = response.invite_response?.Info?.Data?.reference;
  ...
  //     } else {
  //       console.error('No OTP reference found in response:', response);
  //       console.error('Expected path: invite_response.Info.Data.reference');
  //       throw new Error('No OTP reference received from server');
  //     }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate to next step (skip OTP for now)
